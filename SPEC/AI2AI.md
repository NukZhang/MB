...
### 开发环境设置
- **技术栈**: 纯前端项目，无需后端服务器
- **依赖管理**: 无外部依赖库，使用原生JavaScript
- **配置文件**: 所有配置位于`config/`目录（JSON格式）

### 本地服务器
使用Python内置HTTP服务器快速启动预览：
...
### 预览方式
1. 启动本地服务器后，在浏览器中访问：
   - `http://localhost:8080` 或 `http://localhost:8000`
2. 或者在IDE中使用预览工具直接打开URL

### IDE预览工具
- **推荐方式**: 在IDE中点击预览按钮，直接打开内置浏览器
...
1. 修改代码
2. 启动或保持本地服务器运行
3. 刷新浏览器或使用IDE预览工具查看效果
4. 每完成一个迭代任务，拉起预览验证功能
